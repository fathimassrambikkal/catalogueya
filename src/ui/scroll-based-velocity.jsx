...
// Container providing scroll velocity context
...
// Row
export function ScrollVelocityRow({ children, baseVelocity = 5, direction = 1, className, ...props }) {
  const sharedVelocityFactor = useContext(ScrollVelocityContext);
  const velocityFactor = sharedVelocityFactor || useTransform(useSpring(useVelocity(useScroll().scrollY), { damping: 50, stiffness: 400 }), (v) => {
    const sign = v < 0 ? -1 : 1;
...
// Row implementation
function ScrollVelocityRowImpl({ children, baseVelocity, direction, className, velocityFactor }) {
  ...
  return (
    <div ref={containerRef} className={`w-full overflow-hidden ${className || ""}`}>
      <motion.div className="inline-flex transform-gpu will-change-transform" style={{ x }}>
        {Array.from({ length: numCopies }).map((_, i) => (
          <div key={i} ref={i === 0 ? blockRef : null} className="inline-flex shrink-0">
            {children}
          </div>
        ))}
      </motion.div>
