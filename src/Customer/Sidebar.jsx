...
function Sidebar({ activeTab, setActiveTab, onCloseSidebar }) {
  ...
  return (
    <div className="bg-white text-gray-900 h-full p-3 flex flex-col border-r border-gray-200/60  max-w-full z-50 overflow-x-hidden">
      {/* Close Button - Fixed positioning */}
      <div className="absolute top-4 left-4 z-10">
        <button
          onClick={onCloseSidebar}
          className="p-2 rounded-xl bg-white text-gray-500 shadow-md hover:bg-gray-100 transition-colors duration-200 flex-shrink-0"
        >
      ...
      {/* Menu */}
      <nav className="flex-1 flex flex-col gap-1 min-w-0 overflow-hidden">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <FaChevronRight className="text-blue-500 text-xs flex-shrink-0 ml-auto" />
              )}
            </button>
          );
        })}
      ...
      >
        ...
        <span className="ml-3 text-sm font-medium flex-1 text-left truncate min-w-0 overflow-hidden">
          Logout
        </span>
      </button>
      ...
      <div className="mt-4 pt-3 border-t border-gray-200/60 min-w-0 overflow-hidden">
        <div className="flex items-center justify-between text-xs text-gray-600 min-w-0">
          ...
          <div className="w-2 h-2 bg-green-500 rounded-full shadow-md flex-shrink-0 ml-2"></div>
        </div>
      </div>
    </div>
  );
