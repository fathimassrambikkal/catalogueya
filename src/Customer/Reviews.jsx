...
function Reviews() {
  ...
  const renderMainView = () => (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50/30 p-2 xs:p-3 sm:p-6 overflow-x-hidden max-w-full w-full">
      <h1 className="text-base xs:text-lg sm:text-2xl md:text-3xl font-bold text-gray-900 mb-3 xs:mb-4 sm:mb-6 px-1 break-words">
        My Reviews
      </h1>
      ...
      <div className="w-full overflow-x-auto no-scrollbar">
        <div className="flex space-x-2 sm:space-x-4 min-w-max pb-1">
          <button
            ...
            className={`px-4 py-2 rounded-full text-xs xs:text-sm font-medium transition-all duration-200 
      ...
      {/* Awaiting Reviews */}
      {activeTab === 'awaiting' && (
        <div className="w-full max-w-full mx-auto px-1 mt-4">
          <div className="space-y-4 xs:space-y-5 sm:space-y-6">
            <div>
              <h2 className="text-sm xs:text-base sm:text-xl font-semibold text-gray-900 mb-3 xs:mb-4 break-words">
                Awaiting Reviews
              </h2>
              ...
              <div className="space-y-3 xs:space-y-4">
                {awaitingReviews.map((review) => (
                  <div key={review.id}
                    className="relative p-3 xs:p-4 sm:p-6 rounded-xl xs:rounded-2xl
                    ...
                    <button
                      ...
                      p-1 rounded-full bg-white/80 backdrop-blur-lg border border-gray-200/60 shadow flex-shrink-0
                    ...
                    <div className="flex items-start space-x-3 sm:space-x-4 w-full min-w-0">
                      <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center 
                        bg-gray-100/80 border border-gray-200/60 shadow">
                        <span className="text-gray-500 text-xs">Img</span>
                      </div>

                      <div className="flex-1 min-w-0">
                        <div className="mb-2">
                          <span className="inline-block text-xs xs:text-sm text-gray-600 bg-blue-50 px-3 py-1 
                          rounded-full border border-blue-200 break-words max-w-full">
                            {review.reviewCount}
                          </span>
                        </div>

                        <h3 className="font-semibold text-gray-900 text-xs xs:text-sm sm:text-lg mb-1 break-words">
                          {review.productName}
                        </h3>

                        <p className="text-gray-600 text-xs xs:text-sm mb-3 break-words line-clamp-2">
                          {review.description}
                        </p>
                        ...
                        <div className="flex justify-center">
                          <button
                            ...
                            text-xs xs:text-sm font-medium transition-all duration-200 shadow hover:shadow-lg whitespace-nowrap"
                          >
                            Leave a review
                          </button>
                        </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {reviewedProducts.map((product) => (
                  <div key={product.id}
                    className="p-4 sm:p-6 rounded-xl bg-white/80 backdrop-blur-lg border border-gray-200/60 shadow w-full overflow-hidden">

                    <div className="flex items-start space-x-3 sm:space-x-4 min-w-0">
                      <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl bg-gray-100/80 border flex items-center justify-center">
                        <span className="text-gray-500 text-xs">Img</span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0 overflow-hidden">
                        ...
                        <div className="flex gap-2 justify-end flex-wrap">
                          <button className="border border-blue-600 text-blue-600 px-4 py-2 rounded-full text-xs xs:text-sm hover:bg-blue-50 min-w-[60px]">
                            Edit
                          </button>
                          <button className="border border-red-600 text-red-600 px-4 py-2 rounded-full text-xs xs:text-sm hover:bg-red-50 min-w-[60px]">
                            Delete
                          </button>
                        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50/30 p-2 xs:p-3 sm:p-6 overflow-x-hidden max-w-full w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <button
          ...
          className="p-2 rounded-xl bg-white border backdrop-blur-lg shadow min-w-[44px] min-h-[44px]"
        >
        ...
        <h1 className="text-sm xs:text-base sm:text-xl font-bold flex-1 text-center px-2">
          Leave a Review
        </h1>
      ...
      {/* Product Info */}
      <div className="p-4 sm:p-6 rounded-xl bg-white border backdrop-blur-lg shadow mb-6">
        <div className="flex space-x-3 sm:space-x-4 min-w-0">
          <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gray-100 border rounded-xl flex items-center justify-center">
      ...
      <div className="space-y-6">
        ...
        <div>
          <textarea
            ...
            maxLength={300}
            className="w-full h-32 p-4 rounded-xl bg-white border backdrop-blur-lg"
          />
          <div className="text-xs text-right mt-1">{reviewText.length}/300</div>
        </div>

        {/* Star Rating */}
        <div className="p-4 rounded-xl bg-white border backdrop-blur-lg shadow">
        ...
        {/* Submit */}
        <button className="w-full py-4 bg-blue-500 text-white rounded-xl shadow">
          Submit Review
        </button>
