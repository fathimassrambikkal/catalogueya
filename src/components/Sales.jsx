...
function SalesComponent() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setApiProducts(products.data || products); // handle axios response
    };
    fetchProducts();
  }, []);

  // Combine API products with unifiedData as fallback
  const combinedProducts = [...apiProducts, ...unifiedData.salesProducts];
  const limitedProducts = combinedProducts.slice(0, 8);
    ...
    <section className="py-6 sm:py-10 bg-neutral-100 px-3 sm:px-6 md:px-10 lg:px-16 xl:px-24">
      ...
      <div className="flex flex-col md:flex-row items-start justify-between mb-8 sm:mb-12 gap-6">
        ...
        <div className="md:w-1/3 flex justify-start md:justify-end">
          <Link to="/salesproducts" className="relative font-medium text-gray-800 px-2 py-2 overflow-hidden group h-5 sm:h-6 flex items-center gap-1 text-xs sm:text-base">
