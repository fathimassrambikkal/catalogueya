import React, { useState, useEffect, useCallback, useRef } from "react";
import { getSubscribeDetails } from "../api";
import { useTranslation } from "react-i18next";
import { useFixedWords } from "../hooks/useFixedWords";

const ChevronIcon = ({ isRTL = false }) => (
  <svg
    className={`w-3 xs:w-3.5 sm:w-4 h-3 xs:h-3.5 sm:h-4 text-white ${isRTL ? "rotate-180" : ""}`}
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
    <polyline points="9 18 15 12 9 6" />
  </svg>
);

const Pricing = () => {
  const { fixedWords } = useFixedWords();
  const { fixed_words: fw = {} } = fixedWords || {};
  const { i18n } = useTranslation();
  const isRTL = i18n.language === "ar";

  const [tabs, setTabs] = useState([]);
  const [pricing, setPricing] = useState({}); 
  const [activeTab, setActiveTab] = useState(null);
  const [currentFeatures, setCurrentFeatures] = useState([]);
  const [billingCycle, setBillingCycle] = useState("monthly");
  const [isLoading, setIsLoading] = useState(true);
  const [tabDimensions, setTabDimensions] = useState({});
  const tabRefs = useRef([]);
  const containerRef = useRef(null);

  // ================= FETCH DATA =================
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await getSubscribeDetails();
        const data = response?.data;
        
        if (data) {
          const extractedTabs = [];
          Object.keys(data).forEach(key => {
            if (!isNaN(key)) {
              const tabGroup = data[key];
              if (Array.isArray(tabGroup)) {
                extractedTabs.push(...tabGroup);
              }
            }
          });
          
          if (extractedTabs.length > 0) {
            setTabs(extractedTabs);
            setActiveTab(extractedTabs[0]?.key || null);
          } else {
            setTabs([]);
          }
          
          if (data.pricing) {
            setPricing(data.pricing);
          } else {
            setPricing({});
          }
        } else {
          setTabs([]);
          setPricing({});
        }
        
      } catch (err) {
        setTabs([]);
        setPricing({});
      } finally {
        setIsLoading(false);
      }
    };

    fetchData();
  }, []);

  // ================= MEASURE TAB DIMENSIONS =================
  useEffect(() => {
    if (tabs.length > 0) {
      // Small delay to ensure DOM is rendered
      setTimeout(() => {
        const dimensions = {};
        let allMeasured = true;
        
        tabs.forEach((_, index) => {
          const element = tabRefs.current[index];
          if (element) {
            dimensions[index] = {
              width: element.offsetWidth,
              height: element.offsetHeight,
            };
          } else {
            allMeasured = false;
          }
        });
        
        if (allMeasured) {
          setTabDimensions(dimensions);
        }
      }, 50);
    }
  }, [tabs, activeTab]);

  // ================= ACTIVE TAB DATA =================
  const activeTabData = tabs.find(t => t.key === activeTab);

  useEffect(() => {
    if (activeTabData?.benefits?.length) {
      setCurrentFeatures(
        activeTabData.benefits.map(b => ({
          feature: b.title || b.name || b.feature,
          benefit: b.description || b.detail || b.benefit,
        }))
      );
    } else {
      setCurrentFeatures([]);
    }
  }, [activeTabData]);

  // ================= PRICING =================
  const monthlyPrice = pricing.monthly;
  const yearlyPrice = pricing.yearly;
  const yearlySavings = monthlyPrice && yearlyPrice ? monthlyPrice * 12 - yearlyPrice : 0;

  const handleBillingToggle = useCallback(() => {
    setBillingCycle(p => (p === "monthly" ? "yearly" : "monthly"));
  }, []);

  const handleTabClick = useCallback((tabKey) => {
    setActiveTab(tabKey);
  }, []);

  const getTabPosition = () =>
    tabs.findIndex(tab => tab.key === activeTab);

  const getSliderWidth = () => {
    const position = getTabPosition();
    if (position !== -1 && tabDimensions[position]) {
      return tabDimensions[position].width;
    }
    
    // Estimate based on screen size if not measured yet
    if (typeof window !== 'undefined') {
      const width = window.innerWidth;
      if (width < 640) return 80; // xs
      if (width < 768) return 90; // sm
      if (width < 1024) return 100; // md
      return 120; // lg+
    }
    
    return 100; // Default
  };

  const getSliderLeftPosition = () => {
    const position = getTabPosition();
    if (position === -1) return 0;
    
    let left = 0;
    for (let i = 0; i < position; i++) {
      if (tabDimensions[i]) {
        left += tabDimensions[i].width;
      } else {
        // Estimate width based on screen size
        if (typeof window !== 'undefined') {
          const width = window.innerWidth;
          if (width < 640) left += 80;
          else if (width < 768) left += 90;
          else if (width < 1024) left += 100;
          else left += 120;
        } else {
          left += 100;
        }
      }
    }
    
    // Add gap between tabs (responsive)
    const gap = typeof window !== 'undefined' && window.innerWidth < 640 ? 4 : 8;
    left += position * gap;
    
    return left;
  };

  // Initialize tab refs array
  useEffect(() => {
    tabRefs.current = tabRefs.current.slice(0, tabs.length);
  }, [tabs]);

  // ================= RENDER =================
  if (isLoading) {
    return (
      <section
        dir={isRTL ? "rtl" : "ltr"}
        className="bg-white py-8 xs:py-10 sm:py-12 px-3 xs:px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20 font-inter flex flex-col items-center relative overflow-hidden"
      >
        <div className="animate-pulse space-y-6 xs:space-y-8 w-full max-w-6xl">
          <div className="h-8 xs:h-10 sm:h-12 bg-gray-200 rounded w-2/3 xs:w-1/2 sm:w-1/3 mx-auto"></div>
          <div className="h-8 xs:h-10 bg-gray-200 rounded w-40 xs:w-48 sm:w-56 mx-auto"></div>
          <div className="h-48 xs:h-56 sm:h-64 bg-gray-200 rounded"></div>
        </div>
      </section>
    );
  }

  return (
    <section
      dir={isRTL ? "rtl" : "ltr"}
      className="bg-white py-8 xs:py-10 sm:py-12 px-3 xs:px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20 font-inter flex flex-col items-center relative overflow-hidden"
    >
      {/* TITLE */}
      <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-light text-gray-900 text-center">
        {fw.simple_pricing}
      </h1>

      {/* BILLING TOGGLE - Only show if we have pricing data */}
      {monthlyPrice && yearlyPrice && (
        <div className="flex items-center gap-2 xs:gap-3 sm:gap-4 my-6 xs:my-8 p-1.5 xs:p-2 rounded-xl xs:rounded-2xl bg-white/80 backdrop-blur-lg border border-gray-200/60">
          <span className={`text-xs xs:text-sm font-medium ${billingCycle === "monthly" ? "text-blue-600" : "text-gray-500"}`}>
            {fw.monthly}
          </span>
          <button onClick={handleBillingToggle} className={`relative w-14 xs:w-16 h-7 xs:h-8 rounded-full ${billingCycle === "yearly" ? "bg-blue-500" : "bg-gray-300"}`}>
            <div
              className={`absolute top-1 xs:top-1.5 w-5 xs:w-6 h-5 xs:h-6 bg-white rounded-full transition-all ${
                billingCycle === "yearly"
                  ? isRTL ? "left-1 xs:left-1.5" : "right-1 xs:right-1.5"
                  : isRTL ? "right-1 xs:right-1.5" : "left-1 xs:left-1.5"
              }`}
            />
          </button>
          <span className={`text-xs xs:text-sm font-medium ${billingCycle === "yearly" ? "text-blue-600" : "text-gray-500"}`}>
            {fw.yearly}
          </span>
        </div>
      )}

      {/* PRICE - Only show if we have pricing data */}
      {monthlyPrice && billingCycle === "monthly" ? (
        <div className="text-center mb-4 xs:mb-6 space-y-1 xs:space-y-2">
          <h2 className="text-xl xs:text-2xl sm:text-3xl font-bold text-blue-500">
            {monthlyPrice} {fw.qar}
          </h2>
          <p className="text-xs xs:text-sm text-gray-500">{fw.per_month}</p>
        </div>
      ) : yearlyPrice && billingCycle === "yearly" ? (
        <div className="text-center mb-4 xs:mb-6 space-y-1 xs:space-y-2">
          <h2 className="text-xl xs:text-2xl sm:text-3xl font-bold text-blue-600">
            {monthlyPrice && (
              <span className="line-through text-gray-400 mr-1 xs:mr-2 text-base xs:text-lg">
                {monthlyPrice * 12} {fw.qar}
              </span>
            )}
            {yearlyPrice} {fw.qar}
          </h2>
          <p className="text-xs xs:text-sm text-gray-500">
            {fw.per_year } {yearlySavings > 0 && `(${yearlySavings} ${fw.qar} saved)`}
          </p>
        </div>
      ) : !isLoading && !monthlyPrice && !yearlyPrice && (
        <div className="text-center mb-4 xs:mb-6 text-gray-500 text-sm xs:text-base">
          Pricing information not available
        </div>
      )}

      {/* Benefits Section */}
      <h2 className="text-xl xs:text-2xl sm:text-3xl md:text-4xl font-semibold text-gray-800 text-center mb-3 xs:mb-4">
        {fw.benefits_of_subscription}
      </h2>

      {/* TABS */}
      {tabs.length > 0 ? (
        <div 
          ref={containerRef}
          className="relative flex items-center justify-center gap-1 xs:gap-2 mb-8 xs:mb-10 sm:mb-12 bg-white/70 rounded-xl xs:rounded-2xl p-1 xs:p-2 backdrop-blur shadow-[inset_1px_1px_2px_rgba(255,255,255,0.8),inset_-1px_-1px_2px_rgba(0,0,0,0.05)]"
        >
          {/* Sliding Background - Dynamic Width */}
          {tabs.length > 0 && activeTab !== null && (
            <div 
              className={`absolute top-1 xs:top-2 bottom-1 xs:bottom-2 rounded-xl xs:rounded-2xl bg-blue-500 transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)]`}
              style={{ 
                width: `${getSliderWidth()}px`,
                left: isRTL ? 'auto' : `${getSliderLeftPosition()}px`,
                right: isRTL ? `${getSliderLeftPosition()}px` : 'auto'
              }}
            />
          )}
          
          {tabs.map((tab, index) => (
            <button
              key={tab.key}
              ref={el => {
                tabRefs.current[index] = el;
              }}
              onClick={() => handleTabClick(tab.key)}
              className={`relative text-center px-2 xs:px-3 sm:px-4 md:px-5 py-1.5 xs:py-2 sm:py-3 rounded-xl xs:rounded-2xl text-[10px] xs:text-xs sm:text-sm font-medium transition-all duration-300 whitespace-nowrap z-10 ${
                activeTab === tab.key 
                  ? "text-white" 
                  : "text-gray-700 hover:text-blue-500"
              }`}
              data-index={index}
              data-tab-key={tab.key}
            >
              {tab.key}
            </button>
          ))}
        </div>
      ) : (
        !isLoading && (
          <div className="mb-8 xs:mb-10 sm:mb-12 text-center text-gray-500 text-sm xs:text-base">
            No subscription features available
          </div>
        )
      )}

      {/* Main Card Section */}
      {activeTabData && (
        <div className="w-full max-w-6xl mb-3 xs:mb-4">
          <div className="bg-white/90 backdrop-blur-xl rounded-2xl xs:rounded-3xl p-4 xs:p-6 sm:p-8 md:p-10 lg:p-12 flex flex-col md:flex-row gap-6 xs:gap-8 sm:gap-10 border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.04)]">
            {/* Left Content */}
            <div className={`w-full md:w-1/2 text-center md:text-left ${isRTL ? 'md:text-right' : ''} space-y-4 xs:space-y-6`}>
              <span className="inline-block bg-blue-50 text-blue-500 text-xs xs:text-sm font-medium px-2 xs:px-3 py-1 xs:py-1.5 rounded-full">
                {(activeTabData.key || "").toUpperCase()}
              </span>
              <h3 className="text-lg xs:text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 leading-tight">
                {activeTabData.title}
              </h3>
              <p className="text-gray-700 text-sm xs:text-base leading-relaxed max-w-md mx-auto md:mx-0">
                {activeTabData.description}
              </p>
            </div>

            {/* Right Content - Feature Card */}
            <div className="w-full md:w-1/2">
              <div className="bg-white/90 backdrop-blur-xl text-gray-900 rounded-2xl xs:rounded-3xl p-4 xs:p-6 sm:p-8 border border-white/60 min-h-[240px] xs:min-h-[280px] sm:min-h-[300px] flex flex-col justify-center shadow-[8px_8px_25px_rgba(0,0,0,0.08),-8px_-8px_25px_rgba(255,255,255,0.9),inset_1px_1px_2px_rgba(255,255,255,0.8),inset_-1px_-1px_2px_rgba(0,0,0,0.05)]">
                {currentFeatures.length === 0 ? (
                  <div className="text-center text-gray-500 py-6 xs:py-8 text-sm xs:text-base">
                    No features available for this category
                  </div>
                ) : (
                  <ul className="space-y-4 xs:space-y-6 relative z-10">
                    {currentFeatures.map((item, index) => (
                      <li
                        key={index}
                        className={`flex gap-2 xs:gap-3 items-start ${isRTL ? 'flex-row-reverse text-right' : ''}`}
                      >
                        <div className={`
                          flex-shrink-0 
                          w-5 h-5 xs:w-6 xs:h-6 
                          rounded-full 
                          bg-blue-500
                          flex items-center justify-center 
                          ${isRTL ? 'order-2 ml-1 xs:ml-2' : 'mr-1 xs:mr-2'}
                          shadow-[2px_2px_4px_rgba(0,0,0,0.1),-1px_-1px_2px rgba(255,255,255,0.8)]
                        `}>
                          <ChevronIcon isRTL={isRTL} />
                        </div>
                        <div className="flex-1 min-w-0 space-y-0.5 xs:space-y-1">
                          <p className="font-semibold text-gray-800 text-xs xs:text-sm sm:text-base leading-tight">
                            {item.feature}
                          </p>
                          <p className="text-gray-600 text-xs xs:text-sm leading-relaxed">
                            {item.benefit}
                          </p>
                        </div>
                      </li>
                    ))}
                  </ul>
                )}
              </div>
            </div>
          </div>
        </div>
      )}
    </section>
  );
};

export default React.memo(Pricing);