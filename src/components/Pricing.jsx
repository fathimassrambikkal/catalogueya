  ...
  return (
    <section className="bg-neutral-100 py-20 px-4 sm:px-8 md:px-16 font-inter flex flex-col items-center relative overflow-hidden">
      <div className="flex flex-col items-center justify-center mb-8 w-full max-w-3xl">
        <h1 className="text-4xl md:text-5xl font-light text-gray-900 tracking-tight mb-4 text-center">Simple Pricing</h1>
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <button
            ...
            className={`relative w-14 h-7 rounded-full transition-colors duration-500 ${billingCycle === "yearly" ? "bg-blue-500" : "bg-gray-300"}`}
          >
            <motion.div
              layout
              transition={{ type: "spring", stiffness: 250, damping: 30 }}
              className={`absolute top-0.5 w-6 h-6 rounded-full bg-white shadow-md ${billingCycle === "yearly" ? "right-0.5" : "left-0.5"}`}
            />
          </button>
          <span className={`text-sm font-medium ${billingCycle === "yearly" ? "text-blue-600" : "text-gray-500"}`}>Yearly</span>
        </div>
      ...
      <h2 className="text-2xl md:text-3xl font-semibold text-gray-800 mb-4 text-center">Benefits of Subscription</h2>
      ...
      <div className="flex items-center justify-center gap-2 mb-4 bg-white/70 rounded-3xl md:rounded-full p-2 md:p-3 shadow-sm backdrop-blur">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-1 max-w-[120px] sm:max-w-[150px] md:max-w-[180px] lg:max-w-[200px] text-center px-5 sm:px-6 py-2 rounded-full text-[10px] sm:text-[11px] md:text-base font-medium transition-all duration-300 whitespace-nowrap ${activeTab === tab ? "bg-blue-500 text-white shadow-md scale-105" : "bg-white text-gray-700 hover:bg-gray-100 hover:scale-105"}`}
          >
            {tab}
          </button>
        ))}
