...
export default function NewArrivalProductProfile() {
  ...
  const [similarProducts, setSimilarProducts] = useState([]); // kept for future
  ...
  const handleWhatsAppClick = () => {
    if (!product) return;
    const url = `https://wa.me/${whatsappNumber}?text=Hello,%20I'm%20interested%20in%20${encodeURIComponent(
      product.name
    )}`;
...
useEffect(() => {
  ...
  // ⭐ ALWAYS attach company name (important for favourite page display)
  company_name: p.company_name || productData.company_name || "Company",

  // ⭐ Also attach company_id (important for company page navigation)
                ...
                {productImages.map((src, idx) => {
                  ...
                  return (
                    <motion.img
                      ...
                      whileTap={{ scale: 0.9 }}
                      whileHover={{ scale: selectedImage === img ? 1.08 : 1.03 }}
