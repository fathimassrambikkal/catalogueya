...
export default function NewArrivalProductProfile() {
  ...
  const [product, setProduct] = useState(
    newArrivalProducts.find((p) => String(p.id) === String(id))
  );
  ...
  const handleCompanyClick = () => {
    if (product.categoryId && product.companyId) {
      ...
      alert("Company page not available for this product.");
    }
  };
  ...
  const similarProducts = newArrivalProducts
    .filter(
      (sp) =>
        sp.id !== product.id &&
        ...
        <div className="flex flex-col justify-start space-y-6">
          ...
          {product.companyName && (
            <button
              ...
              className="text-base text-gray-600 font-medium hover:underline w-fit"
            >
              by{" "}
