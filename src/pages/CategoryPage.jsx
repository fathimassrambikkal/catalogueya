...
const CompanyCard = memo(({ company, categoryId, navigate }) => (
  <motion.div
    ...
    onClick={() => navigate(`/category/${categoryId}/company/${company.id}`)}
...
const ProductCard = memo(({ product, toggleFavourite, isFav, whatsappNumber, navigate }) => (
  <motion.div
    key={product.id}
    whileHover={{ scale: 1.04 }}
    onClick={() => navigate(`/category/${product.categoryId}/company/${product.companyId}/product/${product.id}`)}
  ...
  >
    ...
    <motion.button
      ...
      whileTap={{ scale: 0.9 }}
      className={`absolute top-2 right-2 z-20 p-2 rounded-full shadow-md transition backdrop-blur-md border ${isFav ? "bg-red-100 text-red-600 border-red-200" : "bg-white text-gray-600 border-white hover:bg-red-50"}`}
    >
    ...
    <div className="relative w-full h-[200px] overflow-hidden">
      <img src={product.img || product.image} alt={product.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const [categoryRes, settingsRes, fixedWordsRes] = await Promise.all([
  ...
  return (
    ...
    >
      ...
      >
        <Suspense fallback={<span>←</span>}><FaArrowLeft className="text-gray-700 text-sm sm:text-md md:text-lg" /></Suspense>
      </button>
      ...
      <div className="relative max-w-7xl mx-auto flex flex-col gap-10 mt-20">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8 mb-10 relative">
          <div className="flex items-center gap-4 md:gap-6 mt-5 md:mt-0 relative">
            <div className="flex items-center gap-3 md:gap-4 ml-10 md:ml-0">
              {category.image && <img src={category.image} alt={category.title} className="w-12 h-12 md:w-16 md:h-16 rounded-full border border-gray-200 shadow-md object-cover" loading="lazy" />}
        ...
        <Suspense fallback={<div className="text-center py-10">Loading cards...</div>}>
          {viewType === "companies" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 place-items-center">
              {sortedCompanies.map(c => <CompanyCard key={c.id} company={c} categoryId={category.id} navigate={navigate} />)}
            </motion.div>
          )}
          ...
          {viewType === "products" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-7 place-items-center">
              {sortedProducts.map(p => <ProductCard key={p.id} product={p} isFav={isFavourite(p.id)} toggleFavourite={toggleFavourite} whatsappNumber={whatsappNumber} navigate={navigate} />)}
            </motion.div>
          )}
        </Suspense>
      </div>
