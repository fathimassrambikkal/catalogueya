...
export default function Sign() {
  ...
  const handleCompanyLogin = async (e) => {
    ...
    try {
      log(" Sending company login request...", {
      ...
      log(" COMPANY LOGIN RESPONSE:", res.data);
      ...
      if (!company || !company.id) {
        throw new Error("Company data missing from login response");
      }

      // Store in LocalStorage
      localStorage.setItem("token", token);
      localStorage.setItem("user", JSON.stringify(company));
      localStorage.setItem("company", JSON.stringify(company)); // For CompanyDashboard compatibility
      ...
      log("Company login successful, redirecting...");
  ...
  const handlePlanSubscribe = (planId) => {
  ...
  return (
    <>
      <CustomerRegisterModal
        ...
        onShowLogin={() => setShowCustomerModal(false)}
      />
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-white/20 shadow-2xl p-8">
            ...
            <div className="flex justify-center mb-8 bg-gray-100/50 rounded-2xl p-1 backdrop-blur-sm">
              <button
                ...
                className={`w-1/2 py-3 rounded-xl font-medium transition-all duration-300 ${loginType === "customer"
              ...
              <button
                ...
                className={`w-1/2 py-3 rounded-xl font-medium transition-all duration-300 ${loginType === "company"
                  ? "bg-white text-blue-600 shadow-lg border border-white/50"
                  : "text-gray-500 hover:text-gray-700 hover:bg-white/30"
                  }`}
              >
