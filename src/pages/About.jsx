  ...
  return (
    <>
      ...
      <section className="bg-gray-50 flex flex-col items-center py-16 px-4 sm:px-6 md:px-16 lg:px-20">
        <span className="inline-block font-medium text-gray-800 px-4 py-2 text-xs sm:text-sm md:text-base bg-white/10 backdrop-blur-2xl border border-white/30 shadow-[0_8px_40px_rgba(0,0,0,0.1)] rounded-full hover:bg-gray-100 transition mb-8 text-center mt-10">
          About Us
        </span>
        ...
        <ScrollVelocityContainer className="w-full py-12 md:py-16">
          <ScrollVelocityRow baseVelocity={3} direction={1} className="flex gap-4 sm:gap-6 md:gap-8 lg:gap-10 justify-start items-center">
            {sliderImages.map((img, i) => <SliderItem key={i} img={img} index={i} />)}
          </ScrollVelocityRow>
      ...
      <section className="flex flex-col items-center justify-center px-4 sm:px-6 md:px-16 lg:px-20 bg-gray-50 py-12 md:py-20">
        ...
        {/* Logos Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 w-full max-w-6xl mx-auto py-8 md:py-16">
          {clientLogos.map((logo, i) => <LogoItem key={i} logo={logo} index={i} />)}
        </div>
      </section>
