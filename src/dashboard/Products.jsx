...
export default function Products({
        ...
        <div className="p-1.5 xs:p-2 flex-grow flex flex-col">
          ...
          <div className="flex items-center justify-between border-t border-gray-50 pt-1 xs:pt-1.5">
            ...
            <button
              ...
              className={`p-0.5 xs:p-1 rounded-full transition-colors ${
                isHidden
  ...
  return (
    <div className="w-full max-w-full min-w-0 px-2 xs:px-3 sm:px-4">
      {/* Header */}
      <div className="w-full min-w-0 mt-3 xs:mt-4 sm:mt-5 md:mt-6 mb-4 xs:mb-5 sm:mb-6">
        <div className="flex flex-col xs:flex-row sm:flex-row items-stretch sm:items-center justify-between gap-2 xs:gap-3 w-full">

          {/* Title */}
          <h2 className="text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl font-semibold text-gray-900 truncate w-full sm:w-auto">
            {fw.our_products} ({products.length})
          </h2>

          {/* Right content */}
          <div className="flex flex-row items-stretch sm:items-center gap-1.5 xs:gap-2 w-full sm:w-auto">

            {/* View Mode Filter */}
            <select
              value={viewMode}
              onChange={(e) => setViewMode(e.target.value)}
              className="bg-white border border-gray-200 text-gray-700 text-[10px] xs:text-xs sm:text-sm md:text-base font-bold rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none p-1.5 xs:p-2 w-1/2 sm:w-auto min-w-0"
            >
      ...
      {/* MODAL */}
      {editingProduct && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex justify-center items-center p-2 xs:p-3 z-[1000]">
          <div
            className="bg-white w-full max-w-[95%] xs:max-w-md sm:max-w-lg md:max-w-xl rounded-lg xs:rounded-xl sm:rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[95vh] xs:max-h-[90vh] animate-in fade-in zoom-in duration-300"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-3 xs:px-4 py-2 xs:py-3 border-b border-gray-100 flex justify-between items-center shrink-0">
              <h2 className="text-xs xs:text-sm sm:text-base font-medium text-gray-900">
                {formData.id
                  ? (fw.edit_product || "Edit Product")
                  : (fw.add_product || "New Product")}
              </h2>
              <button
                className="text-gray-400 hover:text-gray-500 w-6 xs:w-7 h-6 xs:h-7 flex items-center justify-center rounded-full hover:bg-gray-50 transition-colors"
                onClick={closeModal}
              >
                  ...
                  {availableTags.map(tag => {
                    ...
                    return (
                      <button
                        ...
                        `}
                      >
                        <span className={`w-2 xs:w-2.5 sm:w-3 h-2 xs:h-2.5 sm:h-3 rounded-full border flex items-center justify-center ${isActive ? "bg-blue-500 border-blue-500" : "border-gray-300"}`}>
                          {isActive && <FaCheck className="text-white text-[4px] xs:text-[5px] sm:text-[6px]" />}
                        </span>
                        {fw[key] || tag.name}
                      </button>
                    );
