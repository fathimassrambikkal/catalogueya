...
export default function SettingsBillingPage({ onBack }) {
  ...
  if (loading) {
    return (
      <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-white px-4 sm:px-6 lg:px-8 pt-20">
    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-white px-[clamp(0.75rem,4vw,2rem)] sm:px-[clamp(1rem,5vw,2.5rem)] lg:px-[clamp(1.5rem,6vw,3rem)] pt-[clamp(3rem,8vw,4rem)] md:pt-0">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="py-[clamp(1.5rem,4vw,2rem)]">
          ...
          <h1 className={`text-[clamp(1.5rem,5vw,2.5rem)] sm:text-[clamp(2rem,5vw,2.5rem)] font-semibold text-gray-900 tracking-tight ${isRTL ? 'text-right' : ''}`}>
            {fw.billing || "Billing"}
          </h1>
          <p className={`text-[clamp(0.875rem,2vw,1.125rem)] text-gray-600 mt-[clamp(0.25rem,1vw,0.5rem)] ${isRTL ? 'text-right' : ''}`}>
            {fw.manage_subscription_view_payment_history || "Manage your subscription and view your payment history"}
          </p>
        </div>
        ...
        <div className="space-y-[clamp(1.5rem,4vw,2rem)] pb-[clamp(2rem,8vw,4rem)]">
          ...
          <div className="bg-white rounded-[clamp(1.25rem,3vw,2rem)] shadow-sm border border-gray-200 overflow-hidden">
            <div className={`px-[clamp(1.5rem,4vw,2rem)] py-[clamp(1rem,3vw,1.5rem)] border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row ${isRTL ? 'sm:flex-row-reverse' : ''} justify-between items-start sm:items-center gap-[clamp(0.75rem,2vw,1rem]`}>
              <div className={isRTL ? 'text-right' : ''}>
                <h2 className="text-[clamp(1.1rem,2.5vw,1.5rem)] font-semibold text-gray-900">{fw.current_plan || "Current Plan"}</h2>
                <p className="text-[clamp(0.7rem,1.2vw,0.875rem)] text-gray-500 mt-[clamp(0.15rem,0.5vw,0.25rem)]">{fw.operational_tier_allocation || "Operational Tier Allocation"}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className={`px-[clamp(0.5rem,1.5vw,0.75rem)] py-[clamp(0.15rem,0.5vw,0.25rem)] rounded-full text-[clamp(0.5rem,0.8vw,0.625rem)] font-black uppercase tracking-widest ring-1 ${subscription?.subscription_status === 'active' ? 'bg-emerald-50 text-emerald-600 ring-emerald-100' : 'bg-red-50 text-red-600 ring-red-100'
                  ...
                  {subscription?.subscription_status === 'active' ? (fw.active || 'Active') : (subscription?.subscription_status || fw.pending || 'Pending')}
                </span>
              </div>
            </div>
            ...
            <div className="p-[clamp(1.5rem,4vw,2rem)]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-[clamp(2rem,5vw,3rem)]">
                <div>
                  <div className={`flex items-center gap-[clamp(0.75rem,2vw,1rem)] mb-[clamp(0.75rem,2vw,1rem)] ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={isRTL ? 'text-right' : ''}>
                      <p className="text-[clamp(0.6rem,1vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest">{fw.active_plan || "Active Plan"}</p>
                      <p className="text-[clamp(1.25rem,3vw,2rem)] font-black text-gray-900">{subscription?.plan_name || fw.na || 'N/A'}</p>
                    </div>
                  </div>
                  <div className={`space-y-[clamp(0.5rem,1.5vw,0.75rem)] ${isRTL ? 'text-right' : ''}`}>
                    <p className="text-[clamp(0.7rem,1.2vw,0.875rem)] text-gray-500 font-medium">
                      {fw.billed_monthly || "Billed monthly at"} {subscription?.currency || 'QR'} {parseFloat(subscription?.plan_price || 0).toLocaleString()}
                    </p>
                    <p className="text-[clamp(0.7rem,1.2vw,0.875rem)] text-gray-500 font-medium">{fw.unlimited_storage_advanced_analytics || "Unlimited Storage & Advanced Analytics"}</p>
                  </div>
                </div>

                <div className={`flex flex-col justify-between ${isRTL ? 'md:text-left' : 'md:text-right'}`}>
                  <div className={isRTL ? 'text-left' : 'text-right'}>
                    <p className="text-[clamp(0.6rem,1vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest">{fw.next_billing_cycle || "Next Billing Cycle"}</p>
                    <p className="text-[clamp(1rem,2.5vw,1.5rem)] font-bold text-gray-900 mt-[clamp(0.15rem,0.5vw,0.25rem)]">
                      {subscription?.subscription_ends_at ? new Date(subscription.subscription_ends_at).toLocaleDateString(i18n.language === 'ar' ? 'ar-SA' : 'en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : fw.na || 'N/A'}
                    </p>
                  </div>
                  <div className={`flex flex-col sm:flex-row gap-[clamp(0.75rem,2vw,1rem)] mt-[clamp(1rem,3vw,1.5rem)] ${isRTL ? 'sm:flex-row-reverse justify-start md:justify-start' : 'justify-start md:justify-end'}`}>
                    <button
                    ...
                    >
                      {isChangingPlan ? (fw.processing || "Loading...") : (fw.upgrade_plan || "Upgrade Plan")}
                    </button>
                    <button className="px-[clamp(1rem,3vw,1.5rem)] py-[clamp(0.5rem,1.5vw,0.75rem)] bg-white border border-gray-200 text-gray-600 font-bold rounded-2xl hover:bg-gray-50 transition-all active:scale-95 text-[clamp(0.75rem,1.5vw,1rem)]">
                      {fw.cancel_subscription || "Cancel Subscription"}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-[clamp(1.25rem,3vw,2rem)] shadow-sm border border-gray-200 overflow-hidden">
            <div className={`px-[clamp(1.5rem,4vw,2rem)] py-[clamp(1rem,3vw,1.5rem)] border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row ${isRTL ? 'md:flex-row-reverse' : ''} justify-between items-start md:items-center gap-[clamp(1rem,3vw,1.5rem]`}>
              <div className={isRTL ? 'text-right' : ''}>
                <h2 className="text-[clamp(1.1rem,2.5vw,1.5rem)] font-semibold text-gray-900">{fw.billing_history || "Billing History"}</h2>
                <p className="text-[clamp(0.7rem,1.2vw,0.875rem)] text-gray-500 mt-[clamp(0.15rem,0.5vw,0.25rem)]">{fw.transaction_records || "Transaction records"}</p>
              </div>

              <div className={`flex items-center gap-[clamp(0.75rem,2vw,1rem)] w-full md:w-auto ${isRTL ? 'md:flex-row-reverse' : ''}`}>
                <div className="flex items-center bg-gray-100 p-1 rounded-xl">
                  <button
                    onClick={() => setViewMode("table")}
                    className={`p-2 rounded-lg transition-all ${viewMode === 'table' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-400'}`}
                    aria-label={fw.table || "Table View"}
                  >
                    <FaList size={14} />
                  </button>
                  <button
            ...
            <div className="p-[clamp(1.5rem,4vw,2rem)]">
              {invoices.length === 0 ? (
                <div className="py-[clamp(3rem,10vw,5rem)] text-center">
                  ...
                  <p className="text-gray-400 font-bold uppercase tracking-widest text-[clamp(0.7rem,1.5vw,0.875rem)]">{fw.no_transactions || "No transaction records detected"}</p>
                </div>
              ) : viewMode === "table" ? (
                  ...
                  <table className="w-full min-w-[600px]">
                    <thead>
                      <tr className="border-b border-gray-100">
                        <th className={`${isRTL ? 'text-right' : 'text-left'} py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.75rem,2vw,1rem)] text-[clamp(0.6rem,1vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest`}>
                          {fw.date || "Date"}
                        </th>
                        <th className={`${isRTL ? 'text-right' : 'text-left'} py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.75rem,2vw,1rem)] text-[clamp(0.6rem,1vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest`}>
                          {fw.invoice || "Invoice"}
                        ...
                        <th className={`${isRTL ? 'text-left' : 'text-right'} py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.75rem,2vw,1rem)] text-[clamp(0.6rem,1vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest`}>
                          {fw.actions || "Actions"}
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {displayedInvoices.map((inv) => (
                        <tr key={inv.id} className="group hover:bg-gray-50/50 transition-colors">
                          <td className={`py-[clamp(0.75rem,2vw,1.25rem)] px-[clamp(0.75rem,2vw,1rem)] ${isRTL ? 'text-right' : ''}`}>
                            <p className="text-[clamp(0.7rem,1.5vw,0.875rem)] font-bold text-gray-900">
                              {new Date(inv.created_at).toLocaleDateString(i18n.language === 'ar' ? 'ar-SA' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                            </p>
                            <p className="text-[clamp(0.5rem,0.8vw,0.625rem)] font-medium text-gray-400 mt-0.5">
                          ...
                          <td className={`py-[clamp(0.75rem,2vw,1.25rem)] px-[clamp(0.75rem,2vw,1rem)] ${isRTL ? 'text-right' : ''}`}>
                            <span className="text-[clamp(0.7rem,1.5vw,0.875rem)] font-medium text-gray-600">{inv.subscription?.plan?.name || fw.tier_usage || "Tier Usage"}</span>
                          </td>
                          <td className={`py-[clamp(0.75rem,2vw,1.25rem)] px-[clamp(0.75rem,2vw,1rem)] font-black text-gray-900 text-[clamp(0.7rem,1.5vw,0.875rem)] ${isRTL ? 'text-right' : ''}`}>
                            {subscription?.currency || 'QR'} {parseFloat(inv.amount || 0).toFixed(2)}
                          </td>
                          <td className={`py-[clamp(0.75rem,2vw,1.25rem)] px-[clamp(0.75rem,2vw,1rem)] ${isRTL ? 'text-right' : ''}`}>
                            <span className={`inline-flex items-center gap-1.5 px-[clamp(0.5rem,1.5vw,0.75rem)] py-[clamp(0.15rem,0.5vw,0.25rem)] rounded-full text-[clamp(0.5rem,0.8vw,0.625rem)] font-black uppercase tracking-wider border ${getStatusStyles(inv.status)} ${isRTL ? 'flex-row-reverse' : ''}`}>
                              {getStatusIcon(inv.status)}
                              {inv.status === 'paid' ? (fw.paid || 'Paid') : 
                               inv.status === 'pending' ? (fw.pending || 'Pending') : 
                               inv.status === 'unpaid' ? (fw.unpaid || 'Unpaid') : 
                               inv.status}
                            </span>
                          </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[clamp(1rem,3vw,1.5rem)]">
                  {displayedInvoices.map((inv) => (
                    <div key={inv.id} className="bg-white rounded-[clamp(1.5rem,4vw,2rem)] p-[clamp(1rem,3vw,1.5rem)] border border-gray-100 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-300 flex flex-col group relative overflow-hidden">
                      ...
                      <div className={`flex justify-between items-start mb-[clamp(1rem,3vw,1.5rem)] relative ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <span className={`flex items-center gap-1.5 px-[clamp(0.5rem,1.5vw,0.75rem)] py-[clamp(0.15rem,0.5vw,0.25rem)] rounded-full text-[clamp(0.5rem,0.8vw,0.625rem)] font-black border uppercase tracking-widest ${getStatusStyles(inv.status)} ${isRTL ? 'flex-row-reverse' : ''}`}>
                          {getStatusIcon(inv.status)}
                          {inv.status === 'paid' ? (fw.paid || 'Paid') : 
                           inv.status === 'pending' ? (fw.pending || 'Pending') : 
                           inv.status === 'unpaid' ? (fw.unpaid || 'Unpaid') : 
                      ...
                      <div className={`mb-[clamp(1rem,3vw,1.5rem)] ${isRTL ? 'text-right' : ''}`}>
                        <p className="text-[clamp(0.5rem,0.8vw,0.625rem)] font-black text-gray-300 uppercase tracking-[0.2em] mb-1">{fw.invoice || "Invoice"} ID</p>
                        <p className="text-[clamp(1rem,2.5vw,1.5rem)] font-black text-gray-900 font-mono tracking-tighter">{inv.number}</p>
                      </div>

                      <div className={`grid grid-cols-2 gap-[clamp(0.75rem,2vw,1rem)] mb-[clamp(1rem,3vw,1.5rem)] ${isRTL ? 'text-right' : ''}`}>
                        <div>
                          <p className="text-[clamp(0.5rem,0.8vw,0.625rem)] font-black text-gray-300 uppercase mb-1">{fw.date || "Date"}</p>
                          <p className="text-[clamp(0.6rem,1.2vw,0.75rem)] font-bold text-gray-600">
                            {new Date(inv.created_at).toLocaleDateString(i18n.language === 'ar' ? 'ar-SA' : 'en-US')}
                          </p>
                        </div>
                        <div>
                          <p className="text-[clamp(0.5rem,0.8vw,0.625rem)] font-black text-gray-300 uppercase mb-1">{fw.plan || "Plan"}</p>
                          <p className="text-[clamp(0.6rem,1.2vw,0.75rem)] font-bold text-gray-600 line-clamp-1">{inv.subscription?.plan?.name || fw.tier_usage || "Tier Usage"}</p>
                        </div>
                      </div>

                      <div className={`mt-auto pt-[clamp(1rem,3vw,1.5rem)] border-t border-gray-50 flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <div className={`flex flex-col ${isRTL ? 'items-end' : ''}`}>
                          <span className="text-[clamp(0.5rem,0.8vw,0.625rem)] font-black text-gray-300 uppercase">{fw.amount || "Amount"}</span>
                          <span className="text-[clamp(1.25rem,3vw,1.5rem)] font-black text-gray-900">{subscription?.currency || 'QR'} {parseFloat(inv.amount || 0).toFixed(2)}</span>
                        </div>
                        <div className={`flex gap-1.5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                          <button 
                            onClick={() => handleViewInvoice(inv.id)} 
                            className="w-9 h-9 flex items-center justify-center bg-gray-50 hover:bg-white hover:shadow-md text-gray-400 hover:text-blue-600 rounded-xl transition-all"
                            title={fw.view || "View"}
                          >
                          ...
                          <button 
                            onClick={() => handleDownloadInvoice(inv.id, inv.number)} 
                            className="w-9 h-9 flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow-lg shadow-blue-500/20 transition-all"
                            title={fw.download || "Download"}
                          >
                            <IconDownload className="w-4 h-4" />
                          </button>
              ...
              {invoices.length > 5 && !showAllInvoices && (
                <div className="mt-[clamp(1.5rem,4vw,2rem)] text-center">
                  ...
                  >
                    {fw.view_all || "View All"} ({invoices.length - 5} {fw.more || "More"})
                  </button>
                </div>
              )}
            </div>
      ...
      {/* PLANS MODAL - Frosted Glass Minimalist */}
      {showPlansModal && (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 sm:p-6 md:p-8 overflow-hidden">
          ...
          <div className="relative w-full max-w-[900px] max-h-[85vh] bg-white/90 backdrop-blur-2xl rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-white/30 animate-scaleFade">
            ...
            <div className={`px-8 py-6 border-b border-gray-100/50 flex justify-between items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={isRTL ? 'text-right' : ''}>
                <h2 className="text-2xl md:text-3xl font-semibold text-gray-900 tracking-tight">{fw.plans || "Plans"}</h2>
                <p className="text-sm text-gray-500 mt-1 font-medium">{fw.manage_plans_settings || "Manage your plans settings"}</p>
              </div>
              <button
                onClick={() => setShowPlansModal(false)}
                  ...
                  >
                    ...
                    <div className={`flex items-center gap-4 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 uppercase tracking-wider">{plan.interval === 'month' ? fw.monthly || 'Monthly' : fw.yearly || 'Yearly'}</p>
                      </div>
                    </div>

                    {/* Price */}
                    <div className={`mb-4 flex items-baseline gap-1 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
                      <span className="text-2xl font-semibold text-gray-900">{plan.currency} {parseFloat(plan.price).toLocaleString()}</span>
                      <span className="text-xs text-gray-400 font-medium">/{plan.interval === 'month' ? fw.month || 'mo' : fw.year || 'yr'}</span>
                    </div>

                    {/* Feature list - minimal dots */}
                    <div className={`space-y-2 pt-3 border-t border-gray-100 ${isRTL ? 'text-right' : ''}`}>
                      {[
                        fw.full_product_inventory || "Full Product Inventory Management",
                        fw.real_time_analytics || "Real-time Commercial Analytics",
                        fw.prioritized_support || "Prioritized Technical Support",
                        fw.multi_platform_sync || "Multi-platform Synchronization"
                      ].map((feature, fidx) => (
                        <div key={fidx} className={`flex items-center gap-2 text-xs text-gray-600 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div className="px-8 py-6 bg-gray-50/50 border-t border-gray-100/50">
              ...
              <div className={`mt-3 flex items-center justify-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <svg className="w-3.5 h-3.5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <p className="text-xs text-gray-400 font-medium">
                  {fw.plan_change_info || "Plan will change after current billing cycle ends"}
                </p>
              </div>
            </div>
          </div>
      ...
      {isInvoiceModalOpen && selectedInvoice && (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 overflow-hidden">
          ...
          <div className="relative w-full max-w-[500px] bg-white/90 backdrop-blur-2xl rounded-3xl shadow-2xl overflow-hidden border border-white/30 animate-scaleFade">
            <div className={`px-6 py-5 border-b border-gray-100/50 flex justify-between items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
              <h2 className={`text-xl font-semibold text-gray-900 ${isRTL ? 'text-right' : ''}`}>{fw.invoice_details || "Invoice Details"}</h2>
              <button
                onClick={() => setIsInvoiceModalOpen(false)}
            ...
            <div className={`p-6 space-y-5 ${isRTL ? 'text-right' : ''}`}>
              {/* Invoice Number */}
              <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm text-gray-500">{fw.invoice_number || "Invoice Number"}</span>
                <span className="text-sm font-medium text-gray-900">#{selectedInvoice.number}</span>
              </div>

              {/* Status */}
              <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm text-gray-500">{fw.status || "Status"}</span>
                <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${
                  ...
                  {selectedInvoice.status === 'paid' ? (fw.paid || 'Paid') : 
                   selectedInvoice.status === 'pending' ? (fw.pending || 'Pending') : 
                   selectedInvoice.status === 'unpaid' ? (fw.unpaid || 'Unpaid') : 
              ...
              {/* Plan */}
              <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm text-gray-500">{fw.plan || "Plan"}</span>
                <span className="text-sm font-medium text-gray-900">
                  {selectedInvoice.subscription?.plan?.name || fw.tier_usage || "Tier Usage"}
                </span>
              </div>

              {/* Date */}
              <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm text-gray-500">{fw.date || "Date"}</span>
                <span className="text-sm text-gray-900">
                  {new Date(selectedInvoice.created_at).toLocaleDateString(i18n.language === 'ar' ? 'ar-SA' : 'en-US', { 
                    dateStyle: 'medium'
                  })}
                </span>
              </div>
              ...
              <div className="bg-gray-50/80 rounded-xl p-4 space-y-3 mt-2">
                <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <span className="text-xs text-gray-400 uppercase tracking-wider">{fw.subtotal || "Subtotal"}</span>
                  <span className="text-sm font-medium text-gray-900">{selectedInvoice.currency || 'QR'} {parseFloat(selectedInvoice.amount).toFixed(2)}</span>
                </div>
                <div className={`flex items-center justify-between border-b border-gray-200 pb-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <span className="text-xs text-gray-400 uppercase tracking-wider">{fw.tax || "Tax"}</span>
                  <span className="text-sm font-medium text-gray-900">{selectedInvoice.currency || 'QR'} 0.00</span>
                </div>
                <div className={`flex items-center justify-between pt-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <span className="text-sm font-medium text-gray-900">{fw.total || "Total"}</span>
                  <div className={`flex items-baseline gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <span className="text-xs font-medium text-blue-400 uppercase">{selectedInvoice.currency || 'QR'}</span>
                  </div>
                </div>
              </div>
            </div>

            <div className={`px-6 py-4 bg-gray-50/50 border-t border-gray-100/50 flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <button
